...
const SocialAuthProviders = (props) => {
  ...
  const socialAuth = socialAuthProviders.map((provider, index) => {
    ...
    useEffect(() => {
      handleSubmit(provider.loginUrl);
    }, []);
    ...
    return (
      <button
        ...
        key={provider.id}
        type="button"
        className={`btn-social bg-primary border-primary rounded btn-${provider.id} ${index % 2 === 0 ? 'mr-3' : ''}`}
      ...
      >
        {provider.iconImage ? (
          ...
          : (
            <div className="mx-1 rounded" aria-hidden="true">
              <FontAwesomeIcon
